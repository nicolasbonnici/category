...
<div id="modal-create-content" class="modal-body">
    <form role="form" id="newCategoryForm" action="/crud/create/" data-entity="Category" data-view="crud/create.tpl" method="post">
        ...
        <div class="form-group">
            ...
            {% for oCategory in oCategories %}
                <option value="{{oCategory.idcategory}}">{{oCategory.label|safe}}</option>
            {% endfor %}
            </select>
        </div>
        {% endif %}
...
<div class="modal-footer">
    ...
    &nbsp;
    <button type="button" class="btn btn-lg btn-primary ui-sendform refreshOnCallback sendNotificationOnCallback" data-form="#newCategoryForm" title="Enregistrer cette category">{{tr['save']}}</button>
</div>
